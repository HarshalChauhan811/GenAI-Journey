...
import { exec } from "child_process";  // node js executable command --> child process executable
import { promisify } from "util";   // promise return krke degi 
import os from 'os' // hamare paltform me kaunsa platfom hamne install kiya window hai macbook hai 
...
async function executeCommand({command}) {   
    ...
    if(stderr){
        return `Error: ${stderr}`  // command mekuch error aa jaye tab 
    }
...
// tool ka description dena padega 
...
const availableTools = {
   executeCommand              // ek hi tool available hai 
}
